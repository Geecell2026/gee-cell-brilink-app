...
import { createBranchSalaryClaim, type PayrollActionState } from "@/actions/payroll";
...
export function BranchClaimForm({ employees, branches }: { employees: Employee[]; branches: Branch[] }) {
  const [state, formAction, pending] = useActionState(createBranchSalaryClaim, initialState);
  ...
  return (
    <form action={formAction} className="grid grid-cols-3 gap-4 rounded-lg border border-neutral-200 bg-white p-4">
      ...
      <div className="space-y-1">
        <label className={labelClass}>Cabang Pembayar</label>
        <select name="payingBranchId" required className={inputClass} defaultValue="">
          <option value="" disabled>
            Pilih cabang
          </option>
          {branches.map((b) => (
      ...
      <div className="space-y-1">
        <label className={labelClass}>Cabang Penanggung</label>
        <select name="owingBranchId" required className={inputClass} defaultValue="">
          <option value="" disabled>
            Pilih cabang
          </option>
          {branches.map((b) => (
      ...
      >
        {pending ? "Menyimpan..." : "Simpan Klaim"}
      </button>
    </form>
  );
}
